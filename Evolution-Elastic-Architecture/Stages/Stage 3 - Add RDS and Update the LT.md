...
In stage 3, the database functionality will be split out from the EC2 instance, with MariaDB being transitioned to an RDS instance running the MySQL Engine.
...
In this sub-stage of the demo, an RDS instance will be provisioned using the subnet group to control placement within the VPC. 
Normally, multi-AZ would be used for production, but for now, a single AZ should be used as per the instructions below.
...
The blog will be working, even though MariaDB on the EC2 instance is stopped and disabled
It's now running using RDS  
